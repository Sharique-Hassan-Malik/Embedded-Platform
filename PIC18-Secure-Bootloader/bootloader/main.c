...
static uint8_t app_is_valid(void)
{
    ...
    flash_read_block(META_START + META_SHA256_OFF, stored_hash, 32u);
    ...
    while (addr < APP_START + app_size) {
        ...
        chunk = (uint16_t)((APP_START + app_size - addr) < 64u
                         ? (APP_START + app_size - addr) : 64u);
        flash_read_block(addr, tmp, chunk);
        sha256_update(&ctx, tmp, chunk);
        addr += chunk;
    }
    ...
    /* Compare computed hash to stored hash (constant-time via XOR accumulate). */
    {
        uint8_t diff = 0u;
        uint8_t i;
        for (i = 0u; i < 32u; i++) diff |= (computed_hash[i] ^ stored_hash[i]);
        if (diff) return 0u;
    }

    /* Read public key from protected key page. */
    flash_read_block(KEY_PAGE_START + PUBKEY_OFFSET, pubkey, PUBKEY_LEN);

    /* Verify ECDSA signature. */
    return (p256_verify(pubkey, computed_hash, sig_r, sig_s) == 0) ? 1u : 0u;
}
